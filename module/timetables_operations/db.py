...
def set_db_users(connection) -> None:
    ...
    cursor.execute("create table Users(chatid TEXT PRIMARY KEY, username TEXT, current_use TIMESTAMP)")
...
async def insert_db_user(connection,chatid:str,username:str) -> None:
    ...
    if len(check_exist)<=0: #lo fa solo se non esiste gia'
        cursor.execute("insert into Users(chatid,username,current_use) values(?,?,?)",(chatid,username,time,))
